...
class Kulife(MDApp):
    ...
    def build(self):
        ...
        self.dlbl = MDLabel(
            text="Deuteranopia",
            font_style="H6",
            pos_hint = {'center_x': .53, 'center_y': .45}
        )

        self.tlbl = MDLabel(
            text="Tritanopia",
            font_style="H6",
            pos_hint = {'center_x': .57, 'center_y': .25}
        )

        self.n_percentlbl = MDLabel(
            text="",
            font_style="H6",
    ...
    def show_toast(self, obj):
        ...
        app.root.ids.fl.remove_widget(self.n_percentlbl)
    ...
    def visiontest(self,A):
        ...
        if A==3:
            a1=[str(app.test).count("@@normal")]
            a2=[str(app.test).count("@@protanopia")]
            a3=[str(app.test).count("@@deuteranopia")]
            a4=[str(app.test).count("@@tritanopia")]
            ...
            self.nProgress.value = a1ToInt
            self.pProgress.value = a2ToInt
            self.dProgress.value = a3ToInt
            self.tProgress.value = a4ToInt

            self.n_percentlbl.text = str(a1ToInt) + "%"
            self.p_percentlbl.text = str(a2ToInt) + "%"
            self.d_percentlbl.text = str(a3ToInt) + "%"
            self.t_percentlbl.text = str(a4ToInt) + "%"
            
            app.root.ids.fl.remove_widget(self.spin)
            app.root.ids.fl.remove_widget(self.spinlbl)

            app.root.ids.fl.add_widget(self.nlbl)
            app.root.ids.fl.add_widget(self.nProgress)
            app.root.ids.fl.add_widget(self.n_percentlbl)
        ...
        if A==1:
            ...
            Clock.schedule_once(lambda x: app.test00(n1, p1, d1, t1), 8)
            Clock.schedule_once(lambda x: app.test00(n2, p2, d2, t2), 16)
            Clock.schedule_once(lambda x: app.test00(n3, p3, d3, t3), 24)
            Clock.schedule_once(lambda x: app.test00(n4, p4, d4, t4), 32)
            Clock.schedule_once(lambda x: app.test00(n5, p5, d5, t5), 40)
            Clock.schedule_once(lambda x: app.test00(n6, p6, d6, t6), 48)
            Clock.schedule_once(lambda x: app.test00(n7, p7, d7, t7), 56)
            Clock.schedule_once(lambda x: app.test00(n8, p8, d8, t8), 64)
            Clock.schedule_once(lambda x: app.test00(n9, p9, d9, t9), 72)
            Clock.schedule_once(lambda x: app.test00(n10, p10, d10, t10), 80)
            Clock.schedule_once(lambda x: app.visiontest(2), 88)

    def test00(self, n, p ,d ,t):
        M=app.root.ids.rv5
        ...
        listOfImages=[
            {"viewclass": "I2", "source1": n, "type": "normal"},
        ...
        app.root.ids.fl.add_widget(float_btn)

    def leave(self):
        print("True")
        app.root.ids.rv5.clear_widgets()
